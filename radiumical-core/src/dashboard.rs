...
impl Dashboard {
    ...
    pub fn down(&mut self) {
        if self.in_items {
            let max = self.sections.get(self.cat_idx).map(|(_, items)| items.len().saturating_sub(1)).unwrap_or(0);
    ...
    pub fn left(&mut self) {
        if self.in_items { self.in_items = false; self.item_idx = 0; }
    }
    ...
    pub fn render(&self, f: &mut Frame, area: Rect) {
        ...
        let inner = Rect { x: r.x + 1, y: r.y + 1, width: r.width - 2, height: r.height - 2 };

        let chunks = Layout::default().direction(Direction::Horizontal)
            .constraints([Constraint::Percentage(35), Constraint::Percentage(65)]).split(inner);

        // Left: categories
        let nav_lines: Vec<Line> = self.sections.iter().enumerate().map(|(i, (cat, _))| {
        ...
        let nav_block = Block::default().borders(Borders::RIGHT).border_type(BorderType::Rounded)
        ...
        if let Some((cat_name, items)) = self.sections.get(self.cat_idx) {
            ...
            let detail_block = Block::default().borders(Borders::ALL).border_type(BorderType::Rounded)
