...
pub fn sysinfo() -> String {
    ...
    if let Ok(output) = Command::new("uname").arg("-a").output() {
        out.push_str(&format!("OS: {}", String::from_utf8_lossy(&output.stdout).trim()));
        out.push('\n');
    }
    // CPU
    #[cfg(target_os = "linux")]
    ...
    if out.is_empty() { "No system info available.".into() } else { out }
}

/// List directory contents.
pub fn list_dir(path: &PathBuf) -> String {
    let dir = if path.as_os_str().is_empty() { PathBuf::from(".") } else { path.clone() };
    let mut out = String::new();
    if let Ok(entries) = std::fs::read_dir(&dir) {
        for entry in entries.flatten() {
            ...
            let size_str = if size > 0 { format!(" {:>8}", size) } else { "        ".into() };
...
fn tree_recurse(dir: &std::path::Path, prefix: &str, depth: usize, out: &mut String) {
    ...
    for (i, entry) in entries.iter().enumerate() {
        ...
        if hidden && depth < 3 { continue; } // skip hidden unless deep
