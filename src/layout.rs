    ...
    pub fn render(&self, _area_width: u16, _frame: usize, _markdown: &mut crate::markdown::MarkdownRenderer, show_full: bool) -> Vec<Line<'static>> {
        match &self.kind {
            ...
            BlockKind::Reasoning => {
                ...
                let text = &s[1..];
                let display = if show_full { format!("[思考] {text}") } else { let preview: String = text.chars().take(60).collect(); let dots = if text.chars().count() > 60 { "…" } else { "" }; format!("[思考] {preview}{dots}") };
